...
- **Worker**：执行明确工具调用并报告本机状态。执行任务的 pi 保留自己的模型、会话、扩展和交互终端。
...
需要 Go 1.25+、Node.js、已安装 pi SDK 的 pi、tmux。当前 SDK 与交互式桥接在 pi 0.85.1 上验证；模型需要支持 OpenAI Chat Completions 的流式工具调用。
...
[架构设计](docs/architecture.md) · [运行指南](docs/getting-started.md) · [tmux / psmux 运行方案](docs/runtime-tmux-pi.md)
